...
### Error de estimación del modelo en TEST (Predicción)
...
## Vamos a ver graficamente como sería nuestro árbol de decisión

```{r}
library(rpart)
library(rpart.plot)
fit <- rpart(medv~., data = p_train)
...
final_tree <- extract_workflow(final_fit_dt)
...
tree_db <-
  ...
  set_engine("rpart") %>%
  set_mode("regression") %>%
  fit(medv ~ lstat + rm, data = p_train)

p_train %>%
  ggplot(aes(lstat, rm)) +
  geom_parttree(data = tree_db, aes(fill = medv), alpha = 0.3) +
  geom_jitter(alpha = 0.7, width = 1, height = 0.5, aes(color = medv)) +
